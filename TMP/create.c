					...
					/* array in the code)		*/
{
	...
	int p;
	for(p = 0; p < NUMBS; p++){
		pptr->proc_bs[p].bs_status = BSM_UNMAPPED;
		pptr->proc_bs[p].bs_pid = -1;
		pptr->proc_bs[p].bs_vpno = 4096;	//default is the starting virtual page number as given in paging.h
		pptr->proc_bs[p].bs_npages = 0;
		pptr->proc_bs[p].bs_sem = 0; 
		pptr->proc_bs[p].bs_type = BS_PUBLIC;		//public by default
	}
	...
	//the first four page tables are set to the global page tables
	crpd(pid);
