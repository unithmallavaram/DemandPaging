...
//sc queue
struct sc_block *scptr;
struct sc_block *sc_head;
struct sc_block *sc_tail;
...
*/
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	//initialization required for demand paging

	//initialize frame table
	init_frm();

	//initialize the backing store management
	init_bsm();

	//create global page tables and page directories
	initglpt();
	crpd(NULLPROC);

	//set cr3 register to the pdbr of null process
	write_cr3(proctab[NULLPROC].pdbr);

	//set pfint syscall as the go to point to handle interrupt 14
	set_evec(14, pfintr);
	...
	sc_head = getmem(sizeof(struct sc_block));
	sc_tail = getmem(sizeof(struct sc_block));
