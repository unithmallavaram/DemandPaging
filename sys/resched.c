...
int	resched()
{
	...
	// if the old process owned any dirty frames, write them for the other shared 
	int f, store, pageth;
	for(f=0; f<1024; f++){
			...
			if(frm_tab[f].fr_status == FRM_MAPPED){
				if(frm_tab[f].fr_pid == pid){
					if(frm_tab[f].fr_dirty == 1){
						if(bsm_lookup(pid, NBPG*(frm_tab[f].fr_vpno), &store, &pageth) != SYSERR){
							write_bs(NBPG*(1024+f), store, pageth);
						}
					}
				}
			}
	...
	for(f=0; f<1024; f++){
		if(frm_tab[f].fr_type == FR_PAGE){
			if(frm_tab[f].fr_status == FRM_MAPPED){
				if(frm_tab[f].fr_pid == currpid){
					if(bsm_lookup(currpid, NBPG*(frm_tab[f].fr_vpno), &store, &pageth) != SYSERR){
						read_bs(NBPG*(1024+f), store, pageth);
					}
				}
			}
		}
	...
	//if any frames owned by the old process are dirty, 
	for(f = 0; f<1024; f++){

	}
	//switch the pdbr value
